...
matches_data_preprocessing <- function(data){
  ...
  temp[,`:=`(Match_Result = ifelse(Home_Score + Away_Score >2.5, "Over" ,"Under" )
             ,Total_Score = Home_Score + Away_Score)]
  temp[,`:=`(Result_Over = ifelse(Match_Result=="Over",1,0)
             ,Result_Under = ifelse(Match_Result=="Under",1,0))]
...
details_data_preprocessing <- function(data,matches,which_bets=c('ou'),remove_bookmaker=c('BetfairExchange','PaddyPower'),removeOlderThan=30){
